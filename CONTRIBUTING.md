# Contributing to MA2H

Thanks for helping improve the Multi-agent to Human Protocol. MA2H is a specification — a Markdown spec
...
MA2H is **steward-governed** today (see [GOVERNANCE.md](GOVERNANCE.md)), with the stated intent to move to
...
MA2H versions the spec with SemVer intent:
...
(`https://ma2h.org/schema/vX.Y/...`). A **non-breaking** change keeps the existing `$id` / path; a
...
MA2H uses the [Developer Certificate of Origin](https://developercertificate.org) — not a CLA. Sign off every
commit:

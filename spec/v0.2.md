...
**Version:** 0.2 (Draft) · **Date:** 2026-06-03 · **License:** Apache-2.0 · **Steward:** Autonomy LLC
...
Withdraws an open `ask`. On success returns `200 { "id", "status": "cancelled" }`. If the message is
already terminal, returns `409` with the existing `{ "id", "status", "resolution" }` (the agent reads the
real outcome). Idempotent. Tasks are not cancellable in v0.2.
...
- **Agent → Hub.** Bearer token or API key scoped to one `agent.id`. The Hub MUST reject an envelope whose
  ...
  cross-run access; the Hub MUST bind a message's poll/callback access to the principal that submitted it.

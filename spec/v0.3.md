...
`resolution`, `resolution_id`, and `resolved_at` are additionally bound as top-level fields. The top-level
`defaulted` flag and the routing-only `response.agent` are **not** bound and MUST be treated as advisory: a
conformant agent MUST derive the human-vs-expiry-default distinction from the **bound** `actor`
(`system:default_on_expire` vs a `human:`/`agent:` identity) and `resolution` — never from `defaulted` — and
MUST NOT use `response.agent` for authorization, correlation, or state-key selection (the bound `in_reply_to`
+ `callback_url` are the authenticated routing identity). Before v0.3 the answer `value` for a
